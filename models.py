...
def enumerate_models(formula, to_enum, solver):
    ...
    with Solver(name=solver, bootstrap_with=formula.clauses, use_timer=True) as s:
        ...
        for i, model in enumerate(s.enum_models(), 1):
            test=model[:5*3 + 3*2 + 3*4]
            tx='v {0} 0'.format(' '.join(['{0}{1}'.format('+' if v > 0 else '', v) for v in test]))
            if(tx not in l):
                ctr+=1
                l.append(tx)
                if(ctr==23): 
                    print(model)
                    creation_graphe(3,5,model,"fdfdfd{}".format(ctr))
            tmptx='v {0} 0'.format(' '.join(['{0}{1}'.format('+' if v > 0 else '', v) for v in model]))
